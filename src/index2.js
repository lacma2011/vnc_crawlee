...
(async () => {
  ...
  // Define the crawling function that can be triggered
  async function runCrawl(url) {
    const crawler = new PlaywrightCrawler({
      requestHandler: async ({ page, request }) => {
        ...
        // Keep the browser open after the crawl is done
        // console.log("Crawl completed. Keeping the browser open...");
        // await new Promise(() => {}); // Keeps the script running indefinitely

        // Start a REPL for manual command execution
        const replServer = repl.start({ prompt: "> " });

        // Expose the 'page' object to the REPL context
  ...
  // Expose the runScript1 function to the browser context
  await page.exposeFunction("runScript1", async () => {
    console.log("Running Script 1");
    await runCrawl("https://www.instagram.com/amandamccants/?hl=en");
  });
